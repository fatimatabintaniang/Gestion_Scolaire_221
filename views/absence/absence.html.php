...
<!-- Contenu Principal -->
<div class="flex-1 p-6 md:p-8 rounded-2xl bg-white shadow-xl overflow-y-auto h-[70vh] bg-purple-300">

    <!-- En-tête avec titre et boutons - Fonctionnalités conservées -->
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 ">
        <div>
            <h2 class="text-3xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">Liste des Cours planifier</h2>
                    ...
                    <div class="p-5 pt-6">
                        ...
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <h3 class="text-xl font-bold text-gray-800">
                                    <?= htmlspecialchars($absence["date"] ?? 'Non défini') ?>
                                </h3>
                                <p class="text-sm text-gray-500 mt-1">
                                    <?= htmlspecialchars($absence["semestre"] ?? 'Non défini') ?>
                                </p>
                            </div>
                        ...
                        <div class="mb-4">
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                <?= htmlspecialchars($absence["module"] ?? 'Non assigné') ?>
                            </span>
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                <?= htmlspecialchars($absence["nombre_heures"] ?? 'Non assigné') ?>h
                            </span>
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                <?= htmlspecialchars($absence["professeur"] ?? 'Non assigné') ?>
                            </span>
                        </div>
                    </div>
